...
import type { MemberLoadError } from './useMember';

export function MemberPageLoading({ label = 'Loading member…' }: { label?: string }) {
  return (
...
export function MemberPageError({ error, onRetry }: { error: Exclude<MemberLoadError, null>; onRetry: () => void }) {
  ...
  return (
    <div className="mx-auto max-w-2xl rounded-xl border border-gray-200 bg-white px-6 py-14 text-center shadow-sm">
      ...
      <h1 className="mt-4 text-xl font-bold text-gray-900">{notFound ? 'Member not found' : 'Member could not be loaded'}</h1>
      <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-gray-500">
        {notFound
          ? 'This member may have been deleted or the link may be incorrect.'
          : 'Something went wrong while loading this member. Please try again.'}
      </p>
      <div className="mt-6 flex flex-col justify-center gap-3 sm:flex-row">
        ...
        <Link href="/dashboard/members" className="rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm font-semibold text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">
          Back to Members
        </Link>
      </div>
    </div>
