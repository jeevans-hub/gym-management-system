...
export default function DeleteTrainerDialog({ trainer, onClose, onDeleted }: DeleteTrainerDialogProps) {
  ...
  async function handleDelete() {
    ...
    try {
      const response = await fetch(`/api/trainers/${encodeURIComponent(trainer.trainerId)}`, {
        method: 'DELETE',
      ...
      if (response.status === 404) {
        setError('This trainer no longer exists. Refresh the directory and try again.');
        return;
      }
      if (!response.ok) {
        setError('The trainer could not be deleted. Please try again.');
        return;
      }
  ...
  return (
    <div
      ...
      onMouseDown={(event) => {
        if (event.target === event.currentTarget && !deletingRef.current) onClose();
      }}
    >
      <div
        ref={dialogRef}
        ...
        aria-describedby="delete-trainer-description"
      ...
      >
        ...
        <h2 id="delete-trainer-title" className="mt-4 text-xl font-bold text-gray-900">Delete trainer?</h2>
        <p id="delete-trainer-description" className="mt-2 text-sm leading-6 text-gray-600">
          You are about to permanently delete{' '}
          <strong className="font-semibold text-gray-900">
            {trainer.trainerId} — {trainer.firstName} {trainer.lastName}
          </strong>.
